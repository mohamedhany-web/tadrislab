    ...
    (function(){
        ...
        var platformCurrency = <?php echo json_encode(platform_currency(), 15, 512) ?>;
        var platformCurrencyLabel = <?php echo json_encode(currency_label(), 15, 512) ?>;
        ...
        function updateSummary(data) {
            ...
            var egpSmall = ' <span style="font-size:.75rem;font-weight:700;color:#8A94A6">' + curLabel + '</span>' + (isMonthly ? ' <span style="font-size:.7rem;font-weight:700;color:#8A94A6">/<?php echo e(__('public.per_month')); ?></span>' : '');
            var egpLarge = ' <span style="font-size:.8rem;font-weight:700;color:#8A94A6">' + curLabel + '</span>' + (isMonthly ? ' <span style="font-size:.75rem;font-weight:700;color:#8A94A6">/<?php echo e(__('public.per_month')); ?></span>' : '');
            if (!data || !data.ok) {
        ...
        function quote(opts) {
            ...
            if (opts.requireCoupon && !code) {
                setModalError(<?php echo json_encode(__('landing.checkout.coupon_required'), 15, 512) ?>);
            ...
            return fetch(quoteUrl, {
                ...
                if (res.ok && res.data && res.data.ok) {
                    ...
                    if (code && res.data.discount_amount > 0) {
                        ...
                        closeCouponModal();
                        setMsg(<?php echo json_encode(__('landing.checkout.coupon_success'), 15, 512) ?>, false);
                    } else if (code && !(res.data.discount_amount > 0)) {
                        setModalError(<?php echo json_encode(__('landing.checkout.coupon_invalid'), 15, 512) ?>);
                        setMsg(<?php echo json_encode(__('landing.checkout.coupon_invalid'), 15, 512) ?>, true);
                    } else {
                        setMsg(<?php echo json_encode(__('landing.checkout.price_updated'), 15, 512) ?>, false);
                    }
                ...
                var msg = (res.data && res.data.message) ? res.data.message : <?php echo json_encode(__('landing.checkout.price_error'), 15, 512) ?>;
                setModalError(msg);
                setMsg(msg, true);
                updateSummary(null);
                return null;
            }).catch(function(){
                var fail = <?php echo json_encode(__('landing.checkout.connection_error'), 15, 512) ?>;
                setModalError(fail);
                setMsg(fail, true);
                updateSummary(null);
                return null;
    ...
    (function(){
        ...
        function run() {
            ...
            fd.append('currency', (curEl && curEl.value) ? curEl.value : <?php echo json_encode(platform_currency(), 15, 512) ?>);
    ...
    <?php endif; ?>
<?php /**PATH /Users/cityphone/Documents/tadris lab/resources/views/public/partials/checkout-scripts.blade.php ENDPATH**/ ?>