...
@section('page_title', 'تعديل الحساب')
...
<div class="space-y-5">
    <section class="flex flex-wrap items-end justify-between gap-4">
        <div class="min-w-0">
            <p class="text-xs font-medium text-muted">المالية · الحسابات · تعديل</p>
    ...
    <section class="admin-kpi-grid grid gap-3 sm:grid-cols-2">
        ...
        <article class="rounded-2xl border border-line bg-surface p-4 shadow-soft">
            ...
            <p class="mt-3 text-xs text-muted">نوع الحساب</p>
    ...
    <article class="overflow-hidden rounded-2xl border border-line bg-surface shadow-soft" x-data="{ walletType: '{{ old('type', $wallet->type) }}' }">
        <div class="border-b border-line px-4 py-4 sm:px-5">
            ...
            <p class="mt-0.5 text-xs text-muted">قم بتحديث معلومات الحساب واحفظ التغييرات.</p>
        </div>
        ...
        <form action="{{ route('admin.wallets.update', $wallet) }}" method="POST" class="space-y-6 p-4 sm:p-5">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                    <label class="{{ $labelClass }}" for="type">نوع الحساب <span class="text-rose-500">*</span></label>
            ...
            <div class="flex items-center gap-3 rounded-xl border border-line bg-canvas/40 px-4 py-3">
                ...
                <div>
                    <label for="is_active" class="text-sm font-semibold text-ink">تفعيل الحساب</label>
                    <p class="text-xs text-muted">فعّل هذا الخيار للسماح باستخدام الحساب.</p>
                </div>
            </div>
